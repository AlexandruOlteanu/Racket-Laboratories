...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t
      (and (equal? (rev (num->base n (car Bases))) (num->base n (car Bases))) (all-palindromes? n (cdr Bases)))
  )
)
...
(define (palindromes-to-n n Bases)
  (if (= n 0)
      '(0)
      (if (equal? (all-palindromes? n Bases) #t)
...
(define (number-of-bases n Bases)
    (if (null? Bases) 
        0 
        (+ (convert (equal? (rev (num->base n (car Bases))) (num->base n (car Bases)))) (number-of-bases n (cdr Bases))) 
    )
)

(define (first-b-pal start b)
  (if (>= (number-of-bases start '(2 3 4 5 6 7 8 9 10)) b) 
      start 
